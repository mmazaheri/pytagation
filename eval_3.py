...
if __name__ == '__main__':
    ...
    g_check = np.zeros(np.size(g_frame), int)
    c_check = np.zeros(np.size(c_frame), int)

    for thresh_ind in np.arange(np.size(thresholds)):
        for c_ind in np.arange(np.size(c_frame)):
            # print 'where ',np.where(g_frame == c_frame[c_ind])[0]
            for g_ind in np.where(g_frame == c_frame[c_ind])[0]:
                # print g_check[g_ind], c_check[c_ind]
